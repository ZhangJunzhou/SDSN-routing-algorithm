...
# 根据original的值决定 使用哪个路由算法，返回的是路由表
...
# 返回congestion_pair_list（拥塞链路对），和上个函数生成的all_flow
def Network_flow(G,number,weight,original):
    ...
    for i in range(current_flow.shape[0]):     # shape[0]返回二维数组的行数
        a = np.array(current_flow[i])  # weight中的每一行剩余带宽
        temp = np.where( a< 2560*0.1)      # temp指向的是 每一行中 小于2560*0.1中的数据组成的list
        # print(temp[0])
        for j in temp[0]:
            if i==j:
                continue
...
if __name__ == '__main__':
    ...
    weight=  [[0, 2560, M, M, M, M, M, M, 2560, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M],
         ...
         [2560, M, M, M, M, M, M, 2560, 0, M, M, M, 2560, M, M, M, M, 2560, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M],
         ...
         [M, M, 2560, M, M, M, M, M, M, M, 2560, 0, 2560, M, M, 2560, M, M, M, M, 2560, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M, M],
    ...
    for i in range(0,40):
        ...
        for j in range(0,1000):
            temp_weight = copy.deepcopy(weight)
            congestion_pair_list,all_flow = Network_flow(G, 32+i, temp_weight, True)
            del temp_weight
            a[j] =[len(congestion_pair_list),all_flow]
        original_result[32+i] = a
    ...
    for i in range(0,40):
        ...
        for j in range(0,1000):
            ...
            temp_weight = copy.deepcopy(weight)
            congestion_pair_list,all_flow = Network_flow(G,30+i,temp_weight,False)
            del temp_weight
            a[j] =[len(congestion_pair_list),all_flow]
        result[30+i] = a
